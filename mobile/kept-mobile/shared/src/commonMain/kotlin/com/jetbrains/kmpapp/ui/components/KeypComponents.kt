...
@Composable fun KeypTopBar(back: (() -> Unit)? = null) = Row(Modifier.fillMaxWidth().height(56.dp).padding(horizontal = 16.dp), verticalAlignment = Alignment.CenterVertically) { if (back != null) IconButton(onClick = back) { Icon(Icons.AutoMirrored.Filled.ArrowBack, "뒤로가기") }; Text("KeyP", fontWeight = FontWeight.Bold, color = KeypColors.Primary) }
...
@Composable fun KeypBottomBar(selected: String, onSelect: (String) -> Unit) = Row(Modifier.fillMaxWidth().background(Color.White).navigationBarsPadding().padding(vertical = 8.dp), horizontalArrangement = Arrangement.SpaceEvenly) {
    listOf("feed" to "속보 피드", "home" to "Home", "mypage" to "마이페이지").forEach { (key, label) ->
