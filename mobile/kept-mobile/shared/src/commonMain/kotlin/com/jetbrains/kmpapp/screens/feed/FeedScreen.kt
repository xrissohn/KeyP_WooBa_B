...
fun FeedScreen() {
    ...
    val uriHandler = LocalUriHandler.current; Column(Modifier.fillMaxSize()) {
        KeypTopBar(); Row(
        ...
        ) { CircularProgressIndicator() }; is FeedUiState.Content -> if (current.items.isEmpty()) Box(
            Modifier.fillMaxSize(),
            contentAlignment = androidx.compose.ui.Alignment.Center
        ) { Text("새 소식이 없어요") } else LazyColumn(
                ...
                Card(
                    ...
                    Column(Modifier.padding(16.dp)) {
                        ...
                        Text(
                            ...
                            color = KeypColors.InkSecondary
                        )
                    }; HorizontalDivider(Modifier.padding(vertical = 12.dp)); Row(
                        Modifier.fillMaxWidth(),
                        horizontalArrangement = Arrangement.SpaceBetween
                    ) {
                        TextButton(onClick = { uriHandler.openUri(item.url) }) { Text("원문 보기") }; IconButton(
                        onClick = { vm.onBookmark(item.id) }) {
                        Icon(
                            if (item.bookmarked) Icons.Default.Bookmark else Icons.Default.BookmarkBorder,
                            "북마크"
                        )
                    }
                    }
